...
describe("AuthController (Integration)", () => {
  ...
  describe("POST /auth/login", () => {
    ...
    it("should return 401 on invalid credentials", async () => {
      ...
      // Act
      await request(app.getHttpServer()).post("/auth/login").send(loginRequest).expect(401); // Unauthorized response
    });
  ...
  describe("POST /auth/refresh", () => {
    ...
    it("should return 401 on invalid/expired refresh token", async () => {
      ...
      // Act
      await request(app.getHttpServer()).post("/auth/refresh").send(refreshRequest).expect(401); // Unauthorized response
    });
    ...
    it("should return 401 on missing refreshToken", async () => {
      ...
      jest
        .spyOn(authService, "refreshToken")
        .mockRejectedValue(new UnauthorizedException("Refresh token missing"));
  ...
  describe("POST /auth/logout", () => {
    ...
    it("should pass correlationId to logout service", async () => {
      ...
      // Assert
      expect(authService.logout).toHaveBeenCalledWith(expect.any(String), expect.any(String));
    });
  });
});
