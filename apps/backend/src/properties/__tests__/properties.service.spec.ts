...
describe("PropertiesService", () => {
  ...
  describe("create", () => {
    ...
    it("should throw BadRequestException if title missing", async () => {
      ...
      await expect(
        service.create("user-001", { addressId: "addr-001", title: "Test" }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException if user person missing", async () => {
      ...
      await expect(
        service.create("user-001", { title: "Test", addressId: "addr-001" }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("update", () => {
    ...
    it("should throw BadRequestException for invalid propertyType", async () => {
      ...
      await expect(
        service.update("prop-001", "user-001", { propertyType: "invalid" } as any),
      ).rejects.toThrow(BadRequestException);
    ...
    it("should throw ForbiddenException if user not owner", async () => {
      ...
      await expect(service.update("prop-001", "other-user", {})).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
