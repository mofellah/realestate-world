...
const CITIES = {
  ...
  CH: [
    ...
    { name: "Bern", lat: 46.948, lng: 7.4474, region: "Bern" },
...
export function generateAddress(country_code: "BE" | "NL" | "CH" = "BE"): Address {
  ...
  const geoObject = generateGeoObject(
    "point",
...
export function generateProperty(owner?: User): Property {
  ...
  return {
    ...
    images: Array.from(
      ...
      (_, i) => `https://picsum.photos/800/600?random=${generateId()}&property=${i}`,
    ),
...
export function generateListing(property?: Property, creator?: User): Listing {
  ...
  const paymentTermType =
    type === ListingType.SALE ? PaymentTermType.ONETIME : PaymentTermType.PERIODIC;
